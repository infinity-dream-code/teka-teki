...
function tryCookieName(level) {
    return "mtr19_" + level + "_try";
}

/** Wrong attempts used for a level key like l1..l6. Cookie + file, max of both. */
export function getLevelTries(req, ip, level) {
    const key = tryCookieName(level);
...
export function bumpLevelTry(req, res, ip, level, maxTries = 3) {
    const key = tryCookieName(level);
    const h = hashIp(ip);
    const next = Math.min(maxTries, getLevelTries(req, ip, level) + 1);
    const map = loadTries();
...
/** @deprecated use getLevelTries — kept for dakwa5/6 */
export function getCodeTries(req, ip, level) {
    return getLevelTries(req, ip, level);
}

/** @deprecated use bumpLevelTry — L5/L6 max 2 */
export function bumpCodeTry(req, res, ip, level) {
    return bumpLevelTry(req, res, ip, level, 2);
}
